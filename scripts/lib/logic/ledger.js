...
module.exports = function (db) {
  ...
  function debitor (id, callback) {
    ...
    var sql1 = 'SELECT `debitor`.`group_id` FROM `debitor` WHERE `debitor`.`id`='+id+';';
    db.execute(sql1, function(err, ans){
      if (err) return callback(err)
      var sql2 = 'SELECT `debitor_group`.`account_id` FROM `debitor_group` WHERE `debitor_group`.`id`='+sanitize.escape(ans[0].group_id)+';';
      db.execute(sql2, function(err, ans2){
        if (err) return callback(err)

        var query =
          'SELECT c.inv_po_id, c.trans_id, c.trans_date, c.account_id FROM ((SELECT p.inv_po_id, p.trans_id, p.trans_date, p.account_id FROM posting_journal as p WHERE p.deb_cred_id = ' + id + ' AND p.account_id = '+sanitize.escape(ans2[0].account_id)+') ' +
          'UNION (SELECT g.inv_po_id, g.trans_date, g.trans_id, g.account_id FROM general_ledger as g WHERE g.deb_cred_id = ' + id +'  AND g.account_id = '+sanitize.escape(ans2[0].account_id)+')) as c;';
        ...
        db.execute(query, function (err, rows) {
          ...
          var account_id = ans2[0].account_id;
