...
module.exports = (function (db) {
  ...
  var dict = {
      ...
      "balance" : "credits and debits do not balance. credits: %s != debits: %s",
  ...
  var sql = {
    ...
    balance : "SELECT SUM(`posting_journal`.`creditAmount`) AS `creditAmounts`, SUM(`posting_journal`.`debitAmount`) AS `debitAmounts`, SUM(`posting_journal`.`debitEquiv`) AS `debitEquiv`, SUM(`posting_journal`.`creditEquiv`) AS `creditEquiv` FROM `posting_journal`;",
    // are all the creditor/debitor groups + accounts defined?
    defined : "SELECT `posting_journal`.`id`, `posting_journal`.`arapAccount` FROM `posting_journal` WHERE NOT EXISTS (SELECT `creditor_group`.`id` FROM `creditor_group` WHERE `creditor_group`.`account_id`=`posting_journal`.`arapAccount`) UNION (SELECT `debitor_group`.`id` FROM `debitor_group` WHERE `debitor_group`.`account_id`=`posting_journal`.`arapAccount`);",
    // are all fiscal years defined and not locked?
    fiscal  : "SELECT `fiscal_year`.`id`, `posting_journal`.`fYearID` FROM `posting_journal` LEFT JOIN `fiscal_year` ON `posting_journal`.`fYearID`=`fiscal_year`.`id` WHERE `fiscal_year`.`locked`<>1;",
    // are all periods defined and not locked?
    period  : "SELECT `period`.`id`, `posting_journal`.`month` FROM `posting_journal` LEFT JOIN `fiscal_year` ON `posting_journal`.`month`=`period`.`id` WHERE `period`.`locked`<>1;",
