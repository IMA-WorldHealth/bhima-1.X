...
function queryIncomeExpense (params, dateFrom, dateTo) {
	...
	var requette =
			...
			'SUM(`t`.`credit_equiv`) AS credit_equiv, SUM(`t`.`debit`) AS debit, SUM(`t`.`credit`) AS credit, `t`.`currency_id`, `t`.`description`, `t`.`comment`, `t`.`primary_cash_uuid`, `t`.`document_uuid`, `t`.`inv_po_id`, `o`.`service_txt`, `u`.`first`, `u`.`last` ' +
			'FROM (' +
				'(' +
					...
					'WHERE `posting_journal`.`account_id`= ? AND `posting_journal`.`trans_date` <= ? ' +
				') UNION (' +
					'SELECT `general_ledger`.`project_id`, `general_ledger`.`uuid`, `primary_cash_item`.`primary_cash_uuid`, `primary_cash_item`.`document_uuid`, `general_ledger`.`inv_po_id`, `general_ledger`.`trans_date`, `general_ledger`.`debit_equiv`, ' +
						...
						'WHERE `general_ledger`.`account_id`= ? AND `general_ledger`.`trans_date` <= ? ' +
				')' +
			') AS `t`, account AS a, transaction_type as o, user as u WHERE `t`.`account_id` = `a`.`id` AND `t`.`origin_id` = `o`.`id` AND `t`.`user_id` = `u`.`id` ';

	requette += (params.groupedByMonth) ? 'GROUP BY `t`.`trans_id`, YEAR(`t`.`trans_date`), MONTH(`t`.`trans_date`) ;' : 'GROUP BY `t`.`trans_id` ;';
	return db.exec(requette, [params.account_id, params.dateTo, params.account_id, params.dateTo]);
}
