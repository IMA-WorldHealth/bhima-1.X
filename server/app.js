...
app.get('/available_payment_period/', function (req, res, next) {
  var sql = "SELECT p.id, p.config_tax_id, p.config_rubric_id, p.label, p.dateFrom, p.dateTo, r.label, t.label FROM paiement_period p, config_rubric r, config_tax t WHERE p.config_tax_id = t.id AND p.config_rubric_id = r.id";
  db.exec(sql)
  .then(function (result) {
    res.send(result);
  })
